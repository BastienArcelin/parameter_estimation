...
################# With generators
training_generator = generator.BatchGenerator_dc2_redshift(bands,
                                    images_dir,
                                    list_of_samples, 
...
validation_generator = generator.BatchGenerator_dc2_redshift(bands,
                                    images_dir,
                                    list_of_samples_val, 
                                    total_sample_size=None,
                                    batch_size=batch_size, 
...
test_generator = generator.BatchGenerator_dc2_redshift(bands, 
                                    images_dir,
                                    list_of_samples_val, 
                                    total_sample_size=None,
                                    batch_size=batch_size, 
...
#### Model definition
model_choice = 'full_prob_flipout'#'full_prob_flipout'
...
# Callbacks
saving_path = '/sps/lsst/users/barcelin/TFP/weights/test_dc2/v_redshift_flipout'
...
loading_path = '/sps/lsst/users/barcelin/TFP/weights/test_dc2/v_redshift_flipout/loss/'#test_5
latest = tf.train.latest_checkpoint(loading_path)
net.load_weights(latest)
...
fig.savefig('full_prob/test_distrib_redshift.png')


#fig, axes = plt.subplots(1, 1, figsize=(5, 5))
fig = plt.figure()
nb_of_points = 100
plt.errorbar(training_labels[:nb_of_points,0], K.get_value(out.mean())[:nb_of_points,0], yerr = 2*K.get_value(out.stddev())[:nb_of_points,0],  fmt='.', elinewidth=0.5, label = 'mean +/- 2*stddev')
x = np.linspace(-0.5,3.)#(-1,1)#-0,5
plt.plot(x, x)
plt.legend()
plt.ylim(-0.5,3.5)#(-1,1)#-1,1
plt.title('redshift')
